...
/* ── 舵机控制 ── */
static int g_servo_gpios[VENDING_SERVO_COUNT] = {-1, -1, -1, -1}; /**< 舵机 GPIO 引脚 */
...
static const product_t default_products[VENDING_PRODUCT_COUNT] = {
    ...
    {"Sprite", "$1.50", .color = LV_COLOR_MAKE(0x00, 0xCC, 0x00), LV_SYMBOL_TINT},
    {"Water",  "$1.00", .color = LV_COLOR_MAKE(0x29, 0x65, 0xD5), LV_SYMBOL_TINT},
...
vending_machine_config_t vending_machine_get_default_config(void)
{
    vending_machine_config_t config = {
        ...
        .servo_gpios = {27, 53, 46, 48},  /* 4个商品对应的舵机 GPIO */
